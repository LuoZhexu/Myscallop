...
def draw_line_n_mask(im_size, start_coord, orientation, length, thickness, margin, large_dilation_struct, aa_scale, contrast_scale=1.0):
    ...
    arr = (np.array(miniline_blown_im) * 255).astype(np.uint8)
    im_pil = Image.fromarray(arr)
    resized = im_pil.resize((miniline_shape, miniline_shape),
                            resample=Image.LANCZOS)
    miniline_im = np.array(resized).astype(np.float32) / 255.0
    ...
    #                     (miniline_shape, miniline_shape),
    #                     interp='lanczos').astype(np.float) / 255
    arr_mask = (np.array(minimask_blown_im) * 255).astype(np.uint8)
    im_mask_pil = Image.fromarray(arr_mask)
    resized_mask = im_mask_pil.resize((miniline_shape, miniline_shape),
                                      resample=Image.LANCZOS)
    minimask_im = np.array(resized_mask).astype(np.float32) / 255.0
